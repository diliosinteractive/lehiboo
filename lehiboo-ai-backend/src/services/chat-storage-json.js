/**
 * Chat Storage Service - JSON File (Fallback)
...
export function saveMessage(userId, conversationId, role, content, metadata = {}) {
  ...
  logger.info('Message saved (JSON)', {
    userId,
    conversationId,
...
export function deleteConversation(userId, conversationId) {
  ...
  logger.info('Conversation deleted (JSON)', { userId, conversationId });
...
export function clearHistory(userId) {
  ...
  if (cache.users[userId]) {
    ...
    saveStorage();
    logger.info('History cleared (JSON)', { userId });
    return true;
  }
