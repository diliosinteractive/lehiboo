...
<div class="elementor_search_form">
	<div class="el_search_filters wrap_form_search <?php echo ' '.esc_attr($args['type']); ?> <?php echo ' '.esc_attr($args['column']); ?> " >
		...
		<form enctype="multipart/form-data" method="GET" name="search_event" action="<?php echo esc_attr(get_search_result_page()); ?>" autocomplete="off" autocorrect="off" autocapitalize="none" >
			<?php
			$i = 1;
				...
				if( strpos($key,'pos') !== false ){
					...
					switch ( $args[$key] ) {
						...
						<div class="categories field_search">
							...
							<?php 
							$required = false;
							el_get_taxonomy2('event_cat', 'cat', $selected_cat, $required, $category_included ); 
							?>
						...
						<div class="el_start_date field_search">
							<?php if ( $args['icon'.$i] ) { ?>
								<i class="icon_field <?php echo esc_attr( $args['icon'.$i] ); ?>"></i>
							<?php } ?>
							<input type="text" class="el_select_date form-control" placeholder="<?php esc_attr_e('Start date ...', 'eventlist'); ?>" name="start_date" data-format="<?php echo esc_attr( $format ); ?>" value="<?php echo esc_attr($start_date); ?>" data-firstday="<?php echo esc_attr( $first_day ); ?>" autocomplete="off" autocorrect="off" autocapitalize="none" />
						</div>
						<?php
						break;
						...
						<div class="el_end_date field_search">
							<?php if ( $args['icon'.$i] ) { ?>
								<i class="icon_field <?php echo esc_attr( $args['icon'.$i] ); ?>"></i>
							<?php } ?>
							<input type="text" class="el_select_date form-control" placeholder="<?php esc_attr_e('End date ...', 'eventlist'); ?>" name="end_date" data-format="<?php echo esc_attr( $format ); ?>" value="<?php echo esc_attr($end_date); ?>" data-firstday="<?php echo esc_attr( $first_day ); ?>" autocomplete="off" autocorrect="off" autocapitalize="none" />
						</div>
						<?php
						break;
			...
			if( $list_taxonomy_custom && is_array( $list_taxonomy_custom ) ) {
				foreach( $list_taxonomy_custom as $taxo ) {
					...
					$get_term_val = isset( $_GET[$taxo['taxonomy_custom']] ) ? sanitize_text_field( $_GET[$taxo['taxonomy_custom']] ) : '';
					$name_taxos_register = '';
					...
					<div class="field_search ">
						<select name="<?php echo esc_attr( $taxo['taxonomy_custom'] ) ?>" class="selectpicker">
							...
							<?php foreach( $taxos as $tax ) { 
								?>
								<option value="<?php echo esc_attr( $tax->slug ); ?>" <?php selected( $get_term_val, $tax->slug ); ?> ><?php echo esc_html( $tax->name ); ?></option>
							<?php } ?>
						</select>
					</div>
					<?php
